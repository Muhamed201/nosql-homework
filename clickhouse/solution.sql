...
SELECT
    ...
    -- Нет. В данном случае у нас привязка не только по часам, но и к дню, поэтому он будет выводить количество логов за
    -- определенный час и день. Чтобы исправить данную ситуацию, надо использовать функцию toHour() — у неё привязка только к часам.
